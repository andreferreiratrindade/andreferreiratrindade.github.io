      ...
      _createClass(CriarIgrejaPage, [{
        ...
        value: function buscarUsuarioAdministradorIgreja() {
          ...
          this.usuarioService.RecuperaUsuarioPorEmail(this.formData.value.emailAdministrador).then(function (result) {
            if (result.length > 0) {
              _this3.formData.value.administradorUsuarioId = result[0].usuarioId;
            } else {
              src_app_helpers_handlerError__WEBPACK_IMPORTED_MODULE_8__["HandlerError"].handler("Nenhum usuário encontrado com este e-mail.", _this3.toastCtrl);
            }
          })["catch"](function (error) {
